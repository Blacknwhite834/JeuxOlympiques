export default function Card({ title, description, image, textColor, inverse = false }) {
    const containerClasses = `w-full h-fit flex items-start 2xl:items-center  ${inverse ? "flex-col 2xl:flex-row-reverse" : "flex-col 2xl:flex-row"}`;

  return (
    <div className={containerClasses}>
      <img src={image} alt={title} className="w-fit rounded-[30px] sm:rounded-[50px] 2xl:rounded-none" />
